...
class WebGPUvsMetal(Slide):
    def construct(self):
        ...
        # column headers
        hdr_y = 1.8
        hdr_N = CleanText("N", font_size=20, weight=BOLD, color=GREY_C).move_to(
            LEFT * 5.0 + UP * hdr_y
        )
        hdr_w = CleanText(
            "WebGPU (ms)", font_size=18, weight=BOLD, color=BLUE_B
        ).move_to(LEFT * 2.0 + UP * hdr_y)
        hdr_m = CleanText(
            "Metal (ms)", font_size=18, weight=BOLD, color=ORANGE
        ).move_to(RIGHT * 1.0 + UP * hdr_y)
        hdr_r = CleanText(
            "WebGPU / Metal", font_size=18, weight=BOLD, color=GREY_C
        ).move_to(RIGHT * 4.2 + UP * hdr_y)
